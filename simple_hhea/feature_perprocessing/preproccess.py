...
def main(data: str):
    ...
    for h,r,t in kg:
        ...
        node2rel[h + '+' + t] = r
        node2rel[t + '+' + h] = r
    with open(config.path+'node2same','w', encoding='utf-8') as f:
        json.dump(node2same,f)
    with open(config.path+'rel2same','w', encoding='utf-8') as f:
        json.dump(rel2same,f)
    with open(config.path+'rel2index','w', encoding='utf-8') as f:
        json.dump(rel2index,f)
    with open(config.path+'node2rel','w', encoding='utf-8') as f:
        json.dump(node2rel,f)
    print('same node:%d same rel:%d'%(len(node2same),len(rel2same)))

    # fast data
    with open(config.path+'fast.data','w', encoding='utf-8') as f:
        for h,r,t in kg:
            f.write(ents[h]+' '+ents[t]+'\n')

    # deepwalk data
    with open(config.path+'deepwalk.data','w', encoding='utf-8') as f:
        for h,r,t in kg:
            if h in node2same:
                h = node2same[h]
            if t in node2same:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, help='data',default='icews_wiki')
